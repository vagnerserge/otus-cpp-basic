...
// Преобразование строки IP в массив чисел
...
int main(int argc, char const *argv[])
{
    try
    {
      ...
      if ( !file.is_open() ) {
        ...
        //!!!!! грамотно реализовать сравнение 
        ...
        std::sort( ip_pool.begin(), ip_pool.end(), [] ( const std::vector<std::string> & a, const std::vector<std::string> & b ) {
          return a > b; // Лексикографическое сравнение векторов строк
        } );
        ...
        for ( const auto & ip : ip_pool ) {
          // Соединяем элементы вектора через точку
        ...
        std::cout << "Сортированные IP-адреса записаны в файл ip_filter_sort.tsv" << std::endl;
